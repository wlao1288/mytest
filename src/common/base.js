...
if (cur.includes('github')) {
  root_path = `${window.location.origin}/mytest/root`
} else if (cur.includes('netlify')) {
  root_path = `${window.location.origin}/root`
} else {
  root_path = ''
}
          ...
          let sw_timer = window.setInterval(() => {
            if (navigator.serviceWorker.controller) {
              ...
              navigator.serviceWorker.controller.postMessage({
                type: "download",
                content: ["root", "common", "tripcard"].concat(app)
              });
            }
          }, 300);
        }
...
function navigateToTripCard() {
  window.location.href = root_path + "/tripcard/index.html";
}